...
import { SDK_NOTIFICATIONS_ROLE_ID, PROTOCOL_NOTIFICATIONS_ROLE_ID } from "../utils/constants";
 ...
 */
export async function handleSdkNotificationButton(interaction: ButtonInteraction): Promise<void> {
  try {
    ...
    if (!member) {
      await interaction.editReply({
        content: "❌ Unable to find your user information. Please try again later."
      });
      return;
    }
    ...
    console.log(`User ${interaction.user.tag} (${interaction.user.id}) toggled SDK notifications`);
  } catch (error) {
 ...
 * @param interaction The button interaction
 */
export async function handleProtocolNotificationButton(interaction: ButtonInteraction): Promise<void> {
  try {
    await interaction.deferReply({ ephemeral: true });
    ...
    // Log the action for tracking
    console.log(`User ${interaction.user.tag} (${interaction.user.id}) toggled Protocol notifications`);
  } catch (error) {
